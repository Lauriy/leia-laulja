...
class StreamingFaceGrouper:
    ...
    def check_memory_usage(self) -> float:
        ...
        import resource
        memory_mb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024  # On Linux, ru_maxrss is in KB
        return memory_mb
                        ...
                        try:
                            ...
                            if face_data.get('embedding'):
                                ...
                                if face_count % 1000 == 0:
                                    ...
                                    logger.info(f"Processed {face_count} faces, Memory: {memory_mb:.1f}MB")
                                yield face_data
        ...
        try:
            for face in self.parse_faces_manually(self.results_file):
                ...
                if len(face_buffer) >= self.batch_size:
                    ...
                    memory_mb = self.check_memory_usage()
                    logger.info(f"Processed {total_faces_processed} faces, {len(self.face_clusters)} clusters, Memory: {memory_mb:.1f}MB")
                    ...
                    if memory_mb > self.max_memory_mb:
                        ...
                        for cluster_id in list(self.face_clusters.keys()):
                            self.face_clusters[cluster_id] = self.face_clusters[cluster_id][:1]
                        gc.collect()
