...
def cleanup_orphaned_thumbnails(db_path="face_groups.db", thumbnails_dir="face_thumbnails", dry_run=True):
    ...
    for dir_path in sorted(thumbnails_path.rglob("*"), reverse=True):
        if dir_path.is_dir():
            try:
                ...
                logger.info(f"  Removed empty directory: {dir_path.relative_to(thumbnails_path)}")
            except OSError:
                pass  # Directory not empty, that's fine
    
    logger.info(f"🧹 Removed {empty_dirs_removed} empty directories")

def find_db_files_not_on_disk(db_path="face_groups.db", thumbnails_dir="face_thumbnails"):
    ...
    if missing_files:
        ...
        for i, (face_id, thumbnail_path, cluster_id) in enumerate(missing_files[:10]):
            logger.info(f"  {i+1}. {thumbnail_path} (face_id: {face_id}, cluster: {cluster_id})")
